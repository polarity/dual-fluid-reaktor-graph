...
class SVGUse extends SVGShapeEx
{
    public static function getInstance( $x, $y, $id, $ref, $style = null )
    {
        $use = new SVGUse('<use></use>');

        $use->setX( $x );
        $use->setY( $y );
        $use->setId( $id );
        $use->setStyle($style);
        $use->setAttribute( 'xlink:href', "#".$ref, "http://www.w3.org/1999/xlink" );

        return $use;
    }
}

function append_svg_node( $node, $append )
    {
        ...
        if ( isset( $append ) )
        {
            ...
            {
                $xml = $node->addChild( $append->getName(), ' ' );
            ...
            {
                //add one child
                $xml = $node->addChild( $append->getName(), ( string ) $append . ' ' );
            }
...
class SVGDocument extends SVGShape
{
    ...
    public static function getInstance( $filename = null, $SVGClass = 'SVGDocument' )
    {
        ...
        {
            //create clean SVG
            $svg = new $SVGClass( '<?xml version="1.0" encoding="UTF-8" standalone="no"?><svg xmlns:xlink="http://www.w3.org/1999/xlink"></svg>' );
